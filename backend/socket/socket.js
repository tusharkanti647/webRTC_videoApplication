...
export function initSocket(io) {
  io.on('connection', (socket) => {
    ...
    socket.on('check-rome', async (data) => {
      try {
        if (!data?.romeId) {
          io.to(socket.id).emit('check-rome', {
            status: false,
            socketId: socket.id,
        ...
        const fl = await checkRomeIsCreated(data.romeId);
    ...
    socket.on('join-call', async (data) => {
      try {
        ...
        for (let a = 0; a < fl.rome.participants.length; a++) {
          io.to(fl.rome.participants[a].socketId).emit('user-joined', {
            status: true,
    ...
    socket.on('host-user-mute', async (data) => {
      try {
        ...
        console.log('XXXXXXXX', fl);
        if (!fl.status) {
          io.to(socket.id).emit('host-user-mute', {
            status: false,
            socketId: socket.id,
            message: 'This romeId is not created now',
            romeId: data.romeId,
        ...
        // Notify muted user
        io.to(data.socketId).emit('host-by-user-mute', {
          status: true,
          socketId: socket.id,
          mutedSocketId: data.socketId,
          audioEnabled: data.audioEnabled,
...
-------------------------------------------------------------------*/
    socket.on('user-videoOff', async (data) => {
      try {
        ...
        if (!fl.status) {
          io.to(socket.id).emit('user-videoOff', {
            status: false,
            socketId: socket.id,
            message: 'This romeId is not created now',
            romeId: data.romeId,
