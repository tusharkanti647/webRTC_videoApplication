...
export const register = async (req, res) => {
  try {
    ...
    if (!name || !email || !password || !reyTypePassword) {
      return res
        .status(400)
        .json({ message: 'Something went missing.', missing: true, success: false });
    }
    const existUser = await User.findOne({ email: email });
    if (existUser) {
      return res
        .status(409)
        .json({ message: 'User already exist with this email.', missing: false, success: false });
    }
    ...
    return res
      .status(500)
      .json({ message: 'Internal Server Error.', missing: false, success: false });
  }
};
...
export const login = async (req, res) => {
  try {
    ...
    if (!existUser) {
      return res
        .status(401)
        .json({ message: 'Incorrect credentials.', missing: false, success: false });
    }
...
export const authCheck = (req, res) => {
  ...
  try {
    if (userId) {
      ...
      return res
        .status(401)
        .json({ authenticated: false, message: 'Invalid token', success: false });
    }
  } catch (error) {
    console.log(error);
    res
      .status(500)
      .json({ message: 'Internal Server Error.', authenticated: false, success: false });
  }
};
