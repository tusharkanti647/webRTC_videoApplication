...
export default function VideoMeetComponent() {
  ...
  const removeRemoteUser = (socketId) => {
    ...
    setVideos((prev) => {
      ...
      const updatedVideos = prev.filter((v) => v.socketId !== socketId);
  ...
  const addOrUpdateRemoteStream = ({
    ...
    setVideos((prev) => {
      ...
      if (exists) {
        const updatedVideos1 = prev.map((v) => (v.socketId === socketId ? { ...v, stream } : v));
        return updatedVideos1;
      }
      const updatedVideos = [
        ...prev,
        { socketId, socketId, userName, hostId, audioEnabled, videoEnabled, stream },
      ];
  ...
  const handelOpenMenu = (videoObj) => {
    if (videoObj.socketId === isOpenMenu) {
      setIsOpenMenu('');
    } else {
      setIsOpenMenu(videoObj.socketId);
    }
  };
  ...
  const handleKickUser = async (videoObj) => {
    try {
      ...
      console.error('KICK FAILED:', error.response?.data || error.message);
    }
  };

  //muted the user by host
  const handelMuteUser = async (videoObj) => {
    socket.emit('host-user-mute', {
      romeId: romeId,
      socketId: videoObj.socketId,
      audioEnabled: !videoObj.audioEnabled,
    });
  };
  ...
  const responseHostByUserMute = (data) => {
    ...
    localStreamRef.current?.getAudioTracks().forEach((t) => (t.enabled = data.audioEnabled));
  };
  ...
  const responseRemoteUserMuted = (data) => {
    setVideos((v) =>
      v.map((item) =>
        item.socketId === data.mutedSocketId ? { ...item, audioEnabled: data.audioEnabled } : item,
      ),
    );
  };
  ...
  const responseUserVideoOff = (data) => {
    if (data.status) {
      localStreamRef.current?.getVideoTracks().forEach((t) => (t.enabled = data.videoEnabled));
          ...
          <div className={styles.conferenceView}>
            {videos.map((v) => (
              <div key={v.socketId} className={styles.videoDiv}>
                {currentUserHost && (
                  <button className={styles.video3dot} onClick={() => handelOpenMenu(v)}>
                    <MoreVertIcon
                      className={styles.video3dot}
                      sx={{ fontSize: '40', bgcolor: 'gray' }}
                    />
                  </button>
                )}
                ...
                {!v.audioEnabled && (
                  <MicOffIcon
                    className={currentUserHost ? styles.audioMuteHost : styles.audioMute}
                    sx={{ color: 'balck', fontSize: '40', bgcolor: 'gray' }}
                  />
                )}

                {isOpenMenu === v.socketId && (
                  <div className={styles.videoManuDiv}>
